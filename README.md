...
- `POST /api/system/retention-cleanup` - **Não é uma rota de usuário**, mesmo padrão do `reminder-digest` acima: protegida por `X-Cron-Secret` comparado a `CRON_SECRET` (comparação em tempo constante), chamada por um agendador externo. Apaga linhas de `access_logs` mais antigas que `ACCESS_LOG_RETENTION_DAYS` (padrão: 180 dias) e de `reminder_notifications` mais antigas que `REMINDER_NOTIFICATION_RETENTION_DAYS` (padrão: 90 dias). Workflow semanal em `.github/workflows/retention-cleanup.yml`, reutilizando os mesmos secrets `BACKEND_URL`/`CRON_SECRET` do digest de lembretes.
